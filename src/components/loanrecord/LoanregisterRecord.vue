  ...
  <div>
    <div style="margin-top: 10px; ">
      ...
      <div style="float: right; transform: translateY(-60px)">
        <div
          class="block"
          style="margin-top: 30px; margin-left: 15px; display: inline-block"
        >
          <span
            class="demonstration"
        ...
        <el-button
          ...
          size="medium"
          type="primary"
          style="margin-left: 20px; display: inline-block; margin-right: 10px"
          @click="add"
          round
          >新增借条</el-button
        >
    ...
    <el-table
      ...
      border
    >
      <el-table-column prop="loanid" label="借条编号" width="100">
      </el-table-column>
      <el-table-column prop="productid" label="产品编号" width="130">
      </el-table-column>
      ...
      <el-table-column
        ...
        width="130"
      ></el-table-column>
      <el-table-column prop="operate" label="备注" width="150">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.operate === 'Y' ? 'danger' : 'success'"
            size="medium"
            disable-transitions
            >{{ scope.row.operate === "Y" ? "借物" : "还物" }}</el-tag
          >
        </template>
      </el-table-column>
      <el-table-column prop="" label="操作" class="cell">
        <template slot-scope="scope">
          <el-button
            class="button2 button8"
            size="small"
            @click="returnpro(scope.row)"
    ...
    >
      ...
      >
        ...
        <el-form-item label="借条编号" prop="loanid">
          <el-col :span="18"
            ><el-input v-model="form.loanid" size="small"></el-input
          ></el-col>
        </el-form-item>
        ...
        <el-form-item label="数量" prop="quantity">
          <el-col :span="18">
            <el-input v-model="form.quantity" size="small"></el-input>
          </el-col>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    confirmUser() {
      this.form.borrower = this.tempUser.name;
    ...
    DateTime(time) {
      ...
      this.$axios
        .post(this.$httpUrl + "/loanregister/listdatetime?datetime=" + datetime)
        .then((res) => res.data)
        .then((res) => {
          console.log(res);
          if (res.code === 200) {
    ...
    mod(row) {
      ...
      this.$nextTick(() => {
        ...
        this.form.borrower = row.borrower;
      });
    },
    returnpro(row) {
      this.$axios
    ...
    doSave() {
      this.$axios
        .post(this.$httpUrl + "/loanregister/save", {
    ...
    doMod() {
      this.$axios
        .post(this.$httpUrl + "/loanregister/update", this.form)
        .then((res) => res.data)
        .then((res) => {
          console.log(res);
          if (res.code === 200) {
